 ...
 *
 * MODIFIED 1/8/23 for platform defined EUSART
 */
...
void MX25_init( void )
{
	GPIO_PinModeSet(SL_SPIDRV_EUSART_MX25_CS_PORT, SL_SPIDRV_EUSART_MX25_CS_PIN, gpioModeInputPullFilter, 1);
...
void CS_Low()
{
   GPIO_PinOutClear( SL_SPIDRV_EUSART_MX25_CS_PORT, SL_SPIDRV_EUSART_MX25_CS_PIN );
}

void CS_High()
{
   GPIO_PinOutSet( SL_SPIDRV_EUSART_MX25_CS_PORT, SL_SPIDRV_EUSART_MX25_CS_PIN );
}
...
void InsertDummyCycle( uint8_t dummy_cycle )
{
   ...
   {
	   uint8_t buf = 0xFF;
	   //USART_Tx(USART0, 0xFF);
	   SPIDRV_MTransmitB(sl_spidrv_eusart_mx25_handle, &buf, sizeof(buf));
   }
}
...
bool WaitRYBYReady( uint32_t ExpectTime )
{
    ...
    while( SO == 0 )
#else
    while( GPIO_PinInGet(SL_SPIDRV_EUSART_MX25_RX_PORT, SL_SPIDRV_EUSART_MX25_RX_PIN) == 0 );
#endif
    {
        if( temp > ExpectTime )
        {
